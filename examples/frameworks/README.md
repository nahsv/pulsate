...
steps (run the app server on loopback → point DNS → `pulsate up`). TLS is automatic
...
3. **Validate then serve:** `pulsate validate app.flow && sudo pulsate up app.flow`.
...
— or run `pulsate import nginx /etc/nginx/nginx.conf` to auto-translate.
